...
- **DATEV export neutralizes CSV formulas** — `datev.py:booking_line_to_row` prefixes free-text cells starting with `= + - @` tab/CR with `'` (`neutralize_formula_cell`); structured numeric/date/account columns are excluded so they stay parseable. Keeps attacker-controlled counterparty/description out of Excel/DATEV as active formulas (covered by `tests/test_datev.py::TestFormulaNeutralization`).
- **CSV delimiter is a single literal char** — `GenericCsvMappingRequest.delimiter` is validated against `ALLOWED_CSV_DELIMITERS` (`, ; \t |`); anything longer would make pandas treat `sep` as a regex (ReDoS / event-loop DoS). Do not widen it to free-form input (covered by `tests/test_csv_delimiter.py`).
